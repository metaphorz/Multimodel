...
the time slider ($t=-12\to+24$\,h). Because at $t=-12$ the eye is well east of the
grid (offshore, $\mathrm{ewc}=V_T t<0$), the render domain is extended east into the
Atlantic---sized per storm to $12V_T+90$\,mi---and the map zooms out to show the
approach (Figure~\ref{fig:anim}, left); the storm then sweeps west and the eyewall
reaches the coast near $t=0$ (right), fully clearing the grid by $t=+24$. A moving
...
\caption{Optional storm animation (Holland, Category~5). Left: $t=-12$\,h---the
instantaneous windfield offshore over the Atlantic as the storm approaches, the map
zoomed out over the extended domain. Right: $t=0$---the eye at the coast with the
eyewall over Miami/Fort~Lauderdale. The bottom control bar plays or scrubs the
passage; Reset restores the static peak-wind footprint at the default zoom.}
\label{fig:anim}
\end{figure}
