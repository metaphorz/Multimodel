...
clear all 40 columns. This wider span is a \emph{superset} that changes no result:
because the eye starts at the east edge and moves \emph{west}, every land vertex is
closest to the eye at $t\ge 0$, so the per-vertex peak is always attained inside
$[0,12]$\,h---the extra window only adds weaker (farther-eye) winds. We verified this
directly: the peak footprint over $[-12,+24]$ equals the footprint over the ROA
$[0,12]$ window at all $682$ land vertices (maximum difference $0$). Figure~\ref{fig:grid}
...
A second, \emph{parametric} damage model is provided as \textbf{Logistic: Parameter
Selection}. Rather than the tabulated HAZUS curve it uses a closed-form piecewise
...
\section{Verification and testing}\label{sec:verification}
