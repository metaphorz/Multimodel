...
mode runs this \emph{direct simulation} at the vertex (decay unticked). \textbf{Powell}
has no closed-form field to re-simulate, so its single-point sensitivity instead fits
a \emph{per-vertex second-order response surface} live from the 100 precomputed peaks
at the picked vertex (the same 100-vector data the grid-sensitivity map uses), and the
profiler sweeps the inputs through it; this covers peak wind and \%LC (the duration
metrics, which need a time series, remain live-model only). The panel labels which is
in use---``direct simulation'' vs ``per-vertex RSM''.
