...
\item \textbf{The dominant input flips.} For Category~3 \emph{max} wind, the first-order
Sobol' index of $R_{\max}$ rises from $0.028$ (steady) to $0.597$ (dynamic) while WSP
falls from $0.761$ to $0.261$. In the steady model the worst-hit point is set by how
...
The Sobol' machinery above was verified against an entirely separate
implementation. A statistician, working independently in \textbf{JMP}, exported the
...
$R_{\max}$ carries $S_i=0.82$; at the nearby Dania Beach vertex it is $0.63$, at Key
Biscayne $0.37$, and \emph{on} the track it collapses to $0.02$. No other vertex comes
within $0.10$ of JMP's numbers.
...
at that distance, and WSP duly collapses to $0.008$. This is the \emph{inverse} of the
footprint-level ranking of Section~\ref{sec:sobol}, where WSP is a leading driver. One
off-track vertex overturns the domain-wide answer, which is the clearest possible
argument for the single-point profiler of Section~\ref{sec:metamodel} existing at all.
